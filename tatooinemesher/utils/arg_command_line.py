...
class MyArgParse(argparse.ArgumentParser):
    ...
    def add_common_args(self, project_straight_line=False, constant_long_disc=False):
        ...
        if project_straight_line:
            self.mesher_args.add_argument(
                "--project_straight_line",
                action="store_true",
                help="project cross-sections along a straight line " "(linking cross-section bounds)",
            )
        ...
        self.mesher_args.add_argument(
            ...
            choices=("LINEAR", "B-SPLINE", "AKIMA", "PCHIP", "CUBIC_SPLINE", "BILINEAR", "BICUBIC", "BIVARIATE_SPLINE"),
        )

        self.outfile_args = self.add_argument_group("Output files arguments")

    def add_out_mesh_file(self, is_optional=False):
        self.outfile_args.add_argument(
            "--outfile_mesh" if is_optional else "outfile_mesh",
            help="2D results file. Supports formats: Telemac (*.slf), " "BlueKenue (*.t3s) and LandXML (*.xml)",
        )
        self.outfile_args.add_argument(
            "--lang", help="language for standard variables in output file", default="fr", choices=["en", "fr"]
        )
