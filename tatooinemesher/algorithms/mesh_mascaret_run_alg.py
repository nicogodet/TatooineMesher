...
def mesh_mascaret_run(
    infile_geo: str,
    long_step: float,
    outfile_mesh: str,
    infile_res: Optional[str] = None,
    ...
    """Mesh Mascaret geometry and interpolate results if specified.

    Parameters
    ----------
    infile_geo:
        Mascaret geometry file (*.georef, *.georefC).
    long_step:
        Longitudinal space step (in m).
    outfile_mesh:
        2D results file (*.slf, *.t3s, *.xml).
    infile_res:
        Mascaret results file (*.opt, *.rub).
    lat_step:
    ...
    masc_geo = MascaretGeoFile(infile_geo)
    logger.info("Read %s " % masc_geo)
    # masc_geo.export_shp_lines(infile_geo.replace('.georef', '.shp'))
    ...
    if infile_res:
        masc_res = MascaretFile(infile_res)
