...
class CrossSection:
    ...
    def __init__(self, id, coord, label="Cross-section"):
        ...
        if len(coord) < 2:
            raise TatooineException(f"{label} {id} does not have at least 2 points!")
    ...
    def __repr__(self):
        return f"{self.label} #{self.id} ({self.nb_points} points)"
    ...
    def add_limit(self, line_id, Xt_section, Xt_line, point):
        ...
        self.limits[line_id] = {"Xt_section": Xt_section, "Xt_line": Xt_line, "X": point.x, "Y": point.y, **z_values}
    ...
    def find_and_add_limit(self, constraint_line, dist_max=None):
        ...
        if self.geom.intersects(constraint_line.geom):
            ...
            if isinstance(intersection, MultiPoint):
                logger.warn(
                    f"Intersection between '{self}' and '{constraint_line}' contains multiple points, "
            ...
            if isinstance(intersection, Point):
                ...
                raise TatooineException(
                    f"Intersection between '{self}' and '{constraint_line}' is empty or not supported: {type(intersection)}"
                )
        else:
                ...
                if distance < dist_max:
                    for i, coord in enumerate(constraint_line.coord):
                        ...
                        if dist < dist_max:
                            ...
                            self.add_limit(constraint_line.id, Xt_section, Xt_line, intersection)
                            logger.debug(
                                f"Add a limit with the line {constraint_line.id} after {i} iterations (distance = {dist})"
                            )
                            break
    ...
    def extract_bed(self, bed1_id, bed2_id):
        ...
        if Xt1 > Xt2:
            raise TatooineException(f"Order of beds {bed1_id} and {bed2_id} leads to decreasing Xt values for {self}")
        ...
        if Xt1 not in Xt_section:
            row = np.array(
                [tuple(limit1[var] if var not in ("Xt", "xt") else Xt1 for var in sub_coord.dtype.names)],
        ...
        if Xt2 not in Xt_section:
            row = np.array(
                [tuple(limit2[var] if var not in ("Xt", "xt") else Xt2 for var in sub_coord.dtype.names)],
                dtype=sub_coord.dtype,
            )
            row["xt"] = 1.0
        ...
        if not strictly_increasing(sub_coord["Xt"]):
            ...
            logger.debug(limit1)
            logger.debug(limit2)
            points_to_keep = np.ediff1d(sub_coord["Xt"], to_begin=1.0) != 0.0
    ...
    def get_angles(self):
        ...
        angles = []
        for _i, ((x1, z1), (x2, z2), (x3, z3)) in enumerate(
            zip(self.get_segments(), self.get_segments()[1:], self.get_segments()[2:])
        ):
...
class Bed(Coord):
    ...
    def interp_coord_along_bed_auto(self, lat_step, nb_pts_lat=None):
        ...
        Xt_adm_list = np.linspace(0.0, 1.0, num=nb_pts_lat)
...
class CrossSectionSequence:
    ...
    def from_file(filename, label, field_id=None, project_straight_line=False):
        ...
        if filename.endswith(".i3s"):
            ...
            if shp_type in (shapefile.POLYLINEZ, shapefile.POLYLINEM):
                ...
                raise TatooineException(f"The type of file {filename} is not POINT[Z] or POLYLINEZ[M]")
    ...
    def __repr__(self):
        return list(self.section_list).__repr__()
    ...
    def find_and_add_limits(self, constraint_lines, dist_max):
        ...
        for _i, section in enumerate(self):
            ...
            logger.debug(f"{len(limits)} limits found with lines {list(limits)}")
    ...
    def check_intersections(self):
        ...
        if intersections:
            ...
            for i, j in intersections:
                logger.warn(f"- between '{self[i]}' and '{self[j]}'")
        ...
        for section in self:
            ...
            if section_geom.intersects(axe_geom):
                ...
                if isinstance(intersection, Point):
                    ...
                    raise TatooineException(
                        f"Intersection between '{section}' and the hydraulic axis is not a unique point"
                    )
            else:
                if dist_max is not None:
                    for pos in (0, -1):
            ...
            if section.dist_proj_axe == -1:
                logger.warn(
                    f"{section} do not intersect the hydraulic axis (distance = {section.geom.distance(axe_geom)}m) and is ignored"
                )
