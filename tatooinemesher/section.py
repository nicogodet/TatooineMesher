...
class CrossSection:
        ...
        if self.geom.intersects(constraint_line.geom):
            ...
            if isinstance(intersection, Point):
                ...
                raise TatooineException(
                    "Intersection between '{}' and '{}' is empty or not supported: {}".format(
                ...
                if distance < dist_max:
                    for i, coord in enumerate(constraint_line.coord):
                        ...
                        if dist < dist_max:
                            ...
                            self.add_limit(constraint_line.id, Xt_section, Xt_line, intersection)
                            logger.debug(
                                "Add a limit with the line {} after {} iterations (distance = {})".format(
                                    constraint_line.id, i, dist
                                )
                            )
                            break
    ...
    def extract_bed(self, bed1_id, bed2_id):
        ...
        if Xt1 > Xt2:
            raise TatooineException(
                "Order of beds {} and {} leads to decreasing Xt values for {}".format(bed1_id, bed2_id, self)
            )
    ...
    def get_angles(self):
        ...
        angles = []
        for i, ((x1, z1), (x2, z2), (x3, z3)) in enumerate(
        ...
        for section in self:
            ...
            if section_geom.intersects(axe_geom):
                ...
                if isinstance(intersection, Point):
                    ...
                    raise TatooineException(
                        "Intersection between '{}' and the hydraulic axis " "is not a unique point".format(section)
                    )
            else:
                if dist_max is not None:
                    for pos in (0, -1):
            ...
            if section.dist_proj_axe == -1:
                logger.warn(
                    "{} do not intersect the hydraulic axis (distance = {}m) and is ignored".format(
