...
def mesh_mascaret_run(args):
    ...
    logger.info(f"Read {masc_geo} ")
    # masc_geo.export_shp_lines(args.infile_geo.replace('.georef', '.shp'))
    if not masc_geo.has_ref:
        raise TatooineException(f"The file `{masc_geo.file_name}` does not contain any georeferenced data")
    ...
    for reach_id, reach in masc_geo.reaches.items():
        ...
        if len(section_seq) >= 2:
            ...
            logger.error(f"/!\\ Reach {reach_id} ignored because it does not contain at least 2 sections")
    ...
    if args.infile_res:
        ...
        logger.info(f"Variables 1D available at sections: {varnames_1d}")
        try:
